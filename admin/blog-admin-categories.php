<?php
    ...
    $categories = qa_db_select_with_pending( qas_blog_db_category_nav_selectspec( $editcategoryid, true, false, true ) );
    ...
    if ( isset( $editcategory ) ) {
        ...
        if ( qa_clicked( 'blog_doaddcategory' ) )
            $editcategory = array();

        elseif ( qa_clicked( 'blog_dosavecategory' ) ) {
    ...
    if ( qa_clicked( 'blog_dosaveoptions' ) ) {
        if ( !qa_check_form_security_code( 'admin/blog_categories', qa_post_text( 'code' ) ) ) {
            $securityexpired = true;
        } else {
            qa_set_option( 'blog_allow_no_category', (int) qa_post_text( 'option_blog_allow_no_category' ) );
            qa_set_option( 'blog_allow_no_sub_category', (int) qa_post_text( 'option_blog_allow_no_sub_category' ) );
            $savedoptions = true;
        }
    }
    ...
    if ( qa_clicked( 'docancel' ) ) {
    ...
    } elseif ( qa_clicked( 'blog_dosetmissing' ) ) {
        if ( !qa_check_form_security_code( 'admin/blog_categories', qa_post_text( 'code' ) ) )
            $securityexpired = true;

        else {
            $inreassign = qa_get_category_field_value( 'reassign' );
            qas_blog_db_category_reassign( $editcategory[ 'categoryid' ], $inreassign );
        ...
        elseif ( qa_post_text( 'blog_dodelete' ) ) {

            if ( !$hassubcategory ) {
                $inreassign = qa_get_category_field_value( 'reassign' );
                qas_blog_db_category_reassign( $editcategory[ 'categoryid' ], $inreassign );
                qas_blog_db_category_delete( $editcategory[ 'categoryid' ] );
            ...
            $incategories = qa_db_select_with_pending( qas_blog_db_category_nav_selectspec( $inparentid, true ) );
            ...
            for ( $attempt = 0 ; $attempt < 100 ; $attempt++ ) {
                ...
                $matchcategoryid = qas_blog_db_category_slug_to_id( $inparentid, $inslug ); // query against DB since MySQL ignores accents, etc...
            ...
            if ( empty( $errors ) ) {
                if ( isset( $editcategory[ 'categoryid' ] ) ) { // changing existing category
                    qas_blog_db_category_rename( $editcategory[ 'categoryid' ], $inname, $inslug );

                    $recalc = false;

                    if ( $setparent ) {
                        qas_blog_db_category_set_parent( $editcategory[ 'categoryid' ], $inparentid );
                        $recalc = true;
                    } else {
                        qas_blog_db_category_set_content( $editcategory[ 'categoryid' ], $incontent );
                        qas_blog_db_category_set_position( $editcategory[ 'categoryid' ], $inposition );
                    ...
                    $categoryid = qas_blog_db_category_create( $inparentid, $inname, $inslug );

                    qas_blog_db_category_set_content( $categoryid, $incontent );

                    if ( isset( $inposition ) )
                        qas_blog_db_category_set_position( $categoryid, $inposition );
    ...
    $qa_content[ 'title' ] = qa_lang_html( 'qas_admin/admin_title' ) . ' - ' . qa_lang_html( 'qas_admin/blog_category' );
    ...
    if ( $setmissing ) {
        $qa_content[ 'form' ] = array(
            ...
            'fields'  => array(
                'reassign' => array(
                    'label' => isset( $editcategory )
                        ? qa_lang_html_sub( 'qas_admin/category_no_sub_to', qa_html( $editcategory[ 'title' ] ) )
                        : qa_lang_html( 'qas_admin/category_none_to' ),
                    'loose' => true,
                ),
            ),
            ...
            'buttons' => array(
                'save'   => array(
                    'tags'  => 'id="blog_dosaveoptions"', // just used for qa_recalc_click()
            ...
            'hidden'  => array(
                'blog_dosetmissing' => '1', // for IE
                'edit'              => @$editcategory[ 'categoryid' ],
                'missing'           => '1',
                'code'              => qa_get_form_security_code( 'admin/blog_categories' ),
            ),
        );

        qas_blog_set_up_category_field( $qa_content, $qa_content[ 'form' ][ 'fields' ][ 'reassign' ], 'reassign',
            $categories, @$editcategory[ 'categoryid' ], qa_opt( 'blog_allow_no_category' ), qa_opt( 'blog_allow_no_sub_category' ) );
        ...
        if ( $setparent ) {
            ...
            $childdepth = qas_blog_db_category_child_depth( $editcategory[ 'categoryid' ] );

            qas_blog_set_up_category_field( $qa_content, $qa_content[ 'form' ][ 'fields' ][ 'parent' ], 'parent',
            ...
            if ( $hassubcategory ) {
                ...
                $qa_content[ 'form' ][ 'fields' ][ 'delete' ] = array(
                    'tags'  => 'name="blog_dodelete" id="blog_dodelete"',
                    'label' =>
                        '<span id="reassign_shown">' . qa_lang_html( 'qas_admin/delete_category_reassign' ) . '</span>' .
            ...
            $qa_content[ 'form' ][ 'fields' ][ 'questions' ] = array(
                'label' => qa_lang_html( 'qas_admin/total_posts' ),
                'type'  => 'static',
                'value' => '<a href="' . qas_blog_path_html_plural( qa_category_path_request( $categories, $editcategory[ 'categoryid' ] ) ) . '">' .
                    ( ( $editcategory[ 'qcount' ] == 1 )
                        ? qa_lang_html_sub( 'qas_blog/1_post', '1', '1' )
                        : qa_lang_html_sub( 'qas_blog/x_posts', number_format( $editcategory[ 'qcount' ] ) )
                    ) . '</a>',
            );

            if ( $hassubcategory && !qa_opt( 'blog_allow_no_sub_category' ) ) {
                $nosubcount = qas_blog_db_count_categoryid_posts( $editcategory[ 'categoryid' ] );

                if ( $nosubcount )
                    $qa_content[ 'form' ][ 'fields' ][ 'questions' ][ 'error' ] =
                        strtr( qa_lang_html( 'qas_admin/category_no_sub_error' ), array(
            ...
            qa_set_display_rules( $qa_content, array(
                'position_display' => '!blog_dodelete',
                'slug_display'     => '!blog_dodelete',
                'content_display'  => '!blog_dodelete',
                'parent_display'   => '!blog_dodelete',
                'children_display' => '!blog_dodelete',
                'reassign_display' => 'blog_dodelete',
                'reassign_shown'   => 'blog_dodelete',
                'reassign_hidden'  => '!blog_dodelete',
            ) );
        ...
        $qa_content[ 'form' ] = array(
            ...
            'buttons' => array(
                'save' => array(
                    'tags'  => 'name="blog_dosaveoptions" id="blog_dosaveoptions"',
                    'label' => qa_lang_html( 'main/save_button' ),
                ),

                'add'  => array(
                    'tags'  => 'name="blog_doaddcategory"',
        ...
        if ( count( $categories ) ) {
            ...
            foreach ( $categories as $category )
                if ( !isset( $category[ 'parentid' ] ) )
                    $navcategoryhtml .= '<a href="' . qa_path_html( 'admin/blog/categories', array( 'edit' => $category[ 'categoryid' ] ) ) . '">' .
                        qa_html( $category[ 'title' ] ) . '</a> - ' . qa_lang_html_sub( 'qas_blog/x_posts', $category[ 'qcount' ] ) . '<br/>';
            ...
            $qa_content[ 'form' ][ 'fields' ][ 'blog_allow_no_category' ] = array(
                'label' => qa_lang_html( 'qas_admin/allow_no_category' ),
                'tags'  => 'name="option_blog_allow_no_category"',
                'type'  => 'checkbox',
                'value' => qa_opt( 'blog_allow_no_category' ),
            );

            if ( !qa_opt( 'blog_allow_no_category' ) ) {
                $nocatcount = qas_blog_db_count_categoryid_posts( null );

                if ( $nocatcount )
                    $qa_content[ 'form' ][ 'fields' ][ 'blog_allow_no_category' ][ 'error' ] =
                        strtr( qa_lang_html( 'qas_admin/category_none_error' ), array(
            ...
            $qa_content[ 'form' ][ 'fields' ][ 'blog_allow_no_sub_category' ] = array(
                'label' => qa_lang_html( 'qas_admin/allow_no_sub_category' ),
                'tags'  => 'name="option_blog_allow_no_sub_category"',
                'type'  => 'checkbox',
                'value' => qa_opt( 'blog_allow_no_sub_category' ),
            );
    ...
    if ( qa_get( 'recalc' ) ) {
        ...
        $qa_content[ 'form' ][ 'hidden' ][ 'code_recalc' ] = qa_get_form_security_code( 'admin/blog_recalc' );

        $qa_content[ 'script_rel' ][] = qas_blog_plugin_folder() . '/js/blog-admin.js';
        $qa_content[ 'script_var' ][ 'qa_warning_recalc' ] = qa_lang( 'admin/stop_recalc_warning' );

        $qa_content[ 'script_onloads' ][] = array(
            "qas_blog_recalc_click('dorecalccategories', document.getElementById('blog_dosaveoptions'), null, 'recalc_ok');",
        );
    }
