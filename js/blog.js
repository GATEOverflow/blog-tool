...
$(document).ready(function () {
    function warn_on_leave_feature() {
        ...
        $("input:submit,.qa-form-tall-button-ask").click(function () {
            warn_on_leave = false;
            return true;
        });
...
$(function() {
    if($('body').hasClass('qa-template-blog')){
...
function qas_open_comment_form(targetForm,commentid){
    var $form = $("#"+targetForm+" form");
    $form.find("input[name='reply_to']").remove();
    $('<input>' , {type : 'hidden' , name : 'reply_to' , value : commentid}).appendTo($form);

    return qa_toggle_element(targetForm);
}


function qas_blog_comment_click(commentid, postid, parentid, target)
{
	var params = {};
	...
	qas_blog_ajax_post('click_comment', params,
		function(lines) {
			console.log(lines);
			if (lines[0] == '1') {
